# -*- coding: utf-8 -*-
"""One-time script: bulk-assign НАҚШАКАШӢ to all Grade 8 classes in all academic schools."""
import os
import sys
...
SUBJECT = normalize_subject('Нақшакашӣ')


def grade8_classes_for_school(school):
    names = set(
    ...
    return sorted(c for c in names if class_numeric_part(c) == 8)
...
def main():
    ...
    for school in School.objects.all():
        if not is_academic_school(school):
            skipped_schools += 1
            continue
        classes = grade8_classes_for_school(school)
        for cls in classes:
            ...
            if was_created:
                created += 1
                print(f'  [CREATED] {school.name} — {cls} — {SUBJECT}')
            else:
                changed = False
                ...
                if changed:
                    obj.save()
                    print(f'  [REACTIVATED] {school.name} — {cls} — {SUBJECT}')
                else:
                    print(f'  [EXISTS] {school.name} — {cls} — {SUBJECT}')
                existing += 1
        if classes:
            print(f'{school.name}: {len(classes)} grade-8 class(es) -> {classes}')
