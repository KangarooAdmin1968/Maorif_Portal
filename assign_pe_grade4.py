# -*- coding: utf-8 -*-
"""One-time script: bulk-assign ТАРБИЯИ ҶИСМОНӢ to all Grade 4 classes in all academic schools."""
import os
import sys
...
def grade4_classes_for_school(school):
    names = set(
    ...
    return sorted(c for c in names if class_numeric_part(c) == 4)
...
def main():
    ...
    for school in School.objects.all():
        if not is_academic_school(school):
            skipped_schools += 1
            continue
        classes = grade4_classes_for_school(school)
        for cls in classes:
            ...
            if was_created:
                ...
                existing += 1
        if classes:
            print(f'{school.name}: {len(classes)} grade-4 class(es) -> {classes}')
