# -*- coding: utf-8 -*-
"""One-time remote runner: upload settings.py + sync_grade8.py, execute, restart gunicorn."""
import os
import sys
import time
import paramiko
...
def main():
    ...
    run(c, f'cp -p {PROJECT_DIR}/db.sqlite3 {PROJECT_DIR}/db.sqlite3.bak_sync_grade8')
    ...
    sftp.put(os.path.join(BASE, 'sync_grade8.py'), f'{PROJECT_DIR}/sync_grade8.py')
    print('Uploaded sync_grade8.py', flush=True)
    sftp.close()
    ...
    run(c, f'cd {PROJECT_DIR} && {PY} sync_grade8.py')
    run(c, f'rm -f {PROJECT_DIR}/sync_grade8.py')
