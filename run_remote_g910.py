# -*- coding: utf-8 -*-
"""One-time remote runner: upload settings.py + sync_grades910_updates.py, execute, restart gunicorn."""
import os
import sys
import time
import getpass
...
def main():
    ...
    run(c, f'cp -p {PROJECT_DIR}/db.sqlite3 {PROJECT_DIR}/db.sqlite3.bak_sync_grades910')
    ...
    sftp.put(os.path.join(BASE, 'sync_grades910_updates.py'), f'{PROJECT_DIR}/sync_grades910_updates.py')
    print('Uploaded sync_grades910_updates.py', flush=True)
    sftp.close()

    code = run(c, f'cd {PROJECT_DIR} && {PY} manage.py check')
    if code != 0:
        print('manage.py check FAILED — rolling back settings.py', file=sys.stderr)
        run(c, f'cp -p {PROJECT_DIR}/maorif_portal/settings.py.bak_{ts} {PROJECT_DIR}/maorif_portal/settings.py')
        c.close()
        return 1

    run(c, f'cd {PROJECT_DIR} && {PY} sync_grades910_updates.py')
    run(c, f'rm -f {PROJECT_DIR}/sync_grades910_updates.py')
