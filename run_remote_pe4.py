# -*- coding: utf-8 -*-
"""One-time remote runner: upload settings.py + assign_pe_grade4.py to production, execute, restart."""
import os
import sys
import time
import paramiko
...
def main():
    ...
    run(c, f'cp -p {PROJECT_DIR}/db.sqlite3 {PROJECT_DIR}/db.sqlite3.bak_pe_grade4')
    ...
    sftp.put(os.path.join(BASE, 'assign_pe_grade4.py'), f'{PROJECT_DIR}/assign_pe_grade4.py')
    print('Uploaded assign_pe_grade4.py', flush=True)
    sftp.close()
    ...
    run(c, f'cd {PROJECT_DIR} && {PY} assign_pe_grade4.py')
    run(c, f'rm -f {PROJECT_DIR}/assign_pe_grade4.py')
