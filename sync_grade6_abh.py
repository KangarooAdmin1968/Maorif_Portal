# -*- coding: utf-8 -*-
"""One-time script: deactivate 'АСОСҲОИ БЕХАТАРИИ ҲАЁТ' for all Grade 6 classes
in academic schools. Deactivate only — no deletes."""
import os
import sys
...
RETIRED = {normalize_subject('Асосҳои бехатарии ҳаёт')}
...
def main():
    ...
    for school in School.objects.all():
        ...
        for cs in ClassSubject.objects.filter(school=school, is_active=True):
            if class_numeric_part(cs.class_name) != 6:
                continue
