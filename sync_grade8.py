# -*- coding: utf-8 -*-
"""One-time script: deactivate 'ЗАБОНИ НЕМИСӢ', 'ОДОБИ МУОШИРАТ ВА РӮЗГОРДОРӢ' and
'АСОСҲОИ КАСБУ ҲУНАР' for all Grade 8 classes in academic schools. Deactivate only."""
import os
import sys
...
RETIRED = {
    ...
    normalize_subject('Асосҳои касбу ҳунар'),
}
...
def main():
    ...
    for school in School.objects.all():
        ...
        for cs in ClassSubject.objects.filter(school=school, is_active=True):
            if class_numeric_part(cs.class_name) != 8:
                continue
